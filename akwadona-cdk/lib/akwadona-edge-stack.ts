...
// ─────────────────────────────────────────────────────────────────────
export class AkwadonaEdgeStack extends cdk.Stack {
    public readonly webAclArn: string;

    constructor(scope: Construct, id: string, props?: cdk.StackProps) {
        super(scope, id, props);

        const webAcl = new wafv2.CfnWebACL(this, 'AkwadonaEdgeWebACL', {
            name: 'akwadona-edge-acl',
            description: 'Akwadona CloudFront WAF - managed rules and rate limit per source IP.',
            scope: 'CLOUDFRONT',
            defaultAction: { allow: {} },
            visibilityConfig: {
                cloudWatchMetricsEnabled: true,
                metricName: 'akwadona-edge-acl',
