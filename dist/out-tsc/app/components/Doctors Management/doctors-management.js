...
import { Component, ViewChild, DestroyRef, inject, signal, ChangeDetectionStrategy } from '@angular/core';
...
let DoctorsManagementComponent = class DoctorsManagementComponent {
    tableCmp;
    statusTemplate;
    genderTemplate;
    auth = inject(AuthService);
    doctors = inject(DoctorsService);
    ...
    visibleCols = signal(['name', 'gender', 'bloodGroup', 'department', 'specialization', 'status', 'dutyDays', 'dutyStart', 'dutyEnd']);
    ...
    filters = { pageSize: this.pageSize, lastKey: null };
    activeFilters = { status: null, gender: null };
    filterControls = [
        {
            id: 'status',
            type: 'cycle',
    ...
    onRowEditInit(_) { }
    onRowEditCancel(_) { }
    beginEdit(row, rowIndex, api) {
        const id = this.pkToId(row.PK || '');
        if (id) {
            const originalRow = this._rows().find((r) => this.pkToId(r.PK) === id);
            if (originalRow) {
    ...
    async exportExcel() {
        ...
        const fields = this.columns.map((c) => c.field).filter((f) => Boolean(f));
        const headers = this.columns.map((c) => c.header || c.field);
        const rowsData = dataToExport.map((item) => fields.map((field) => {
    ...
    deleteSelected(deletionOption) {
        ...
        this.confirm.confirm({
            ...
            accept: () => {
                const ids = Array.from(new Set(sel.map((p) => this.pkToId(p.PK)).filter((id) => !!id)));
                this._loading.set(true);
                ...
                const request$ = deletionOption === 'hard'
                    ? this.doctors.hardDeleteDoctor(ids).pipe(map(() => ids.map(() => true)), catchError((err) => {
                        handleUnauthorized(err);
                        return of(ids.map(() => false));
                    }))
                    : forkJoin(ids.map((id) => this.doctors.deleteDoctor(id).pipe(map(() => true), catchError((err) => {
                        handleUnauthorized(err);
                        return of(false);
                    }))));
    ...
    load(e) {
        ...
        const so = e.sortOrder ?? 1;
        const sortChanged = sf !== this.prevSortField || so !== this.prevSortOrder;
        this.prevSortField = sf;
        this.prevSortOrder = so;
        const first = e.first || 0;
        const base = { pageSize: this.pageSize, offset: first, sortField: sf, sortOrder: so };
        if (e.filters?.global?.value)
            base.search = String(e.filters.global.value).trim();
        for (const [k, f] of Object.entries(e.filters || {})) {
            if (k === 'global' || !f?.value)
                continue;
        ...
        if (this.activeFilters.gender)
            base['gender.equals'] = this.activeFilters.gender;
        this.filters = base;
        if (sortChanged)
            this.filters.lastKey = null;
        this.fetch();
    }
    fetch() {
        this._loading.set(true);
...
], DoctorsManagementComponent.prototype, "tableCmp", void 0);
__decorate([
    ViewChild('statusTemplate')
], DoctorsManagementComponent.prototype, "statusTemplate", void 0);
__decorate([
    ViewChild('genderTemplate')
], DoctorsManagementComponent.prototype, "genderTemplate", void 0);
DoctorsManagementComponent = __decorate([
    Component({
...
export { DoctorsManagementComponent };
