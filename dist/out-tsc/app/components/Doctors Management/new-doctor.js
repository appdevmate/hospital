...
let NewDoctor = class NewDoctor {
    ...
    statusOptions = ['senior', 'junior', 'under development', 'associate'];
        ...
        obs.subscribe({
            ...
            error: (err) => {
                ...
                if (kind === 'department')
                    ...
                    this.filteredSpecializationOptions = [];
            }
        });
    }
    ...
    submit() {
        ...
        const payload = {
            ...
            dutyDays: Array.isArray(f.dutyDays) ? f.dutyDays.map((d) => d.toLowerCase()) : [],
    ...
    saveBulk() {
        ...
        const obs = this.bulkType === 'department' ? this.doctorService.bulkCreateDepartments(this.bulkPreview) : this.doctorService.bulkCreateSpecializations(this.bulkPreview);
        obs.subscribe({
            next: (res) => {
    ...
    async parseTwoColFile(file, codeOptional = false) {
        try {
            const ext = file.name.toLowerCase().split('.').pop() || '';
            if (ext === 'csv')
                return this.parseCsvText(await file.text(), codeOptional);
            if (ext === 'xlsx' || ext === 'xls') {
                const XLSX = await import('xlsx');
                const wb = XLSX.read(await file.arrayBuffer(), { type: 'array' });
                const ws = wb.Sheets[wb.SheetNames[0]];
                return this.normalizeRows(XLSX.utils.sheet_to_json(ws, { defval: '' }), codeOptional);
            }
    ...
    parseCsvText(text, codeOptional) {
        const lines = text.split(/\r?\n/).filter((l) => l.trim().length);
        if (!lines.length)
            return { ok: true, rows: [], invalid: [] };
        const split = (row) => row.match(/("([^"]|"")*"|[^,]+)/g)?.map((s) => s.replace(/^"(.*)"$/, '$1').replace(/""/g, '"')) || [];
    ...
    normalizeRows(json, codeOptional) {
        const rows = [];
        const invalid = [];
        for (const r of json) {
