...
let PatientsManagementComponent = class PatientsManagementComponent {
    tableCmp;
    statusTemplate;
    genderTemplate;
    // ── Services ────────────────────────────────────────────────────────
    ...
    visibleCols = signal(['name', 'bloodGroup', 'gender', 'phone', 'qid', 'dob', 'status', 'bedNumber', 'ward']);
    ...
    async exportExcel() {
        ...
        const rows = dataToExport.map((item) => fields.map((field) => {
        ...
        this.confirm.confirm({
            ...
            accept: () => {
                ...
                const request$ = deletionOption === 'hard'
                    ? this.patients.hardDeletePatient(ids).pipe(map(() => ids.map(() => true)), catchError((err) => {
                        handleUnauthorized(err);
                        return of(ids.map(() => false));
                    }))
                    : forkJoin(ids.map((id) => this.patients.deletePatient(id).pipe(map(() => true), catchError((err) => {
                        handleUnauthorized(err);
                        return of(false);
                    }))));
    ...
    load(e) {
        ...
        const base = { pageSize: this.pageSize, offset: e.first || 0, sortField: sf, sortOrder: so };
        ...
        if (this.activeFilters.status === 'active')
            base['status.notEquals'] = 'dead,discharged';
        if (this.activeFilters.status === 'nonactive')
            base['status.equals'] = 'dead,discharged';
    ...
    prepare(rows) {
        const valid = [];
        const skipped = [];
        const lc = (x) => (typeof x === 'string' && x.trim() ? x.trim().toLowerCase() : null);
        const sanitizePhone = (x) => {
...
], PatientsManagementComponent.prototype, "tableCmp", void 0);
__decorate([
    ViewChild('statusTemplate')
], PatientsManagementComponent.prototype, "statusTemplate", void 0);
__decorate([
    ViewChild('genderTemplate')
], PatientsManagementComponent.prototype, "genderTemplate", void 0);
PatientsManagementComponent = __decorate([
    Component({
...
export { PatientsManagementComponent };
