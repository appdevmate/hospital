...
let DashboardComponent = class DashboardComponent {
    ...
    in7DaysStr = this.formatDate(new Date(this.today.getTime() + 7 * 24 * 60 * 60 * 1000));
    thisMonthStr = `${this.today.getFullYear()}-${String(this.today.getMonth() + 1).padStart(2, '0')}`;
    ngOnInit() {
    ...
    loadAdminDashboard() {
        forkJoin({
            ...
            const totalDoctors = doctors.totalCount || 0;
            const totalPatients = patients.totalCount || 0;
    ...
    loadDoctorDashboard(doctorEmail) {
        forkJoin({
