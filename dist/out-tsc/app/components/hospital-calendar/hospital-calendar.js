...
let HospitalCalendarComponent = class HospitalCalendarComponent {
    calendarEl;
    ...
    newEvent = { name: '', description: '', color: '#3B82F6', recurrence: 'none', startTime: null, endTime: null };
    editEvent = { name: '', description: '', color: '#3B82F6', recurrence: 'none', startDate: '', endDate: '', startTime: null, endTime: null };
    colorOptions = [
        { value: '#3B82F6', label: 'Blue' },
        { value: '#10B981', label: 'Green' },
        { value: '#F59E0B', label: 'Amber' },
    ...
    loadDoctorOwnCalendar() {
        ...
        forkJoin({
            ...
            this.cachedDoctors = doctors.data || [];
    ...
    syncDoctorCalendars() {
        ...
        forkJoin({
            ...
            const deletes$ = orphaned.map((c) => this.calendarService.deleteCalendar(c.calendarId).pipe(catchError(() => of(null))));
            const all$ = [...creates$, ...deletes$];
            const proceed = (updatedCalendars) => {
                this.calendarList = updatedCalendars;
                this.syncing = false;
                this.selectFirst();
    ...
    seedDutyShiftsIfEmpty(doctors, calendars) {
    ...
    onCalendarChange() {
        ...
        this.calendarService.getEvents(this.selectedCalendarId).subscribe((events) => {
            this.fcInstance.removeAllEvents();
            events.forEach((e) => this.fcInstance.addEvent({
                id: e.eventId,
                title: e.name,
                start: e.startDate,
                end: e.endDate,
            ...
            this.cdr.detectChanges();
            if (events.some((e) => e.description?.includes('DUTY_SHIFT')))
                return;
            const cal = this.calendarList.find((c) => c.calendarId === this.selectedCalendarId);
            const doctor = this.cachedDoctors.find((d) => cal && d.name === cal.name);
            if (!doctor?.dutyDays?.length || !doctor.dutyStart || !doctor.dutyEnd)
    ...
    loadEvents() {
        if (!this.selectedCalendarId || !this.fcInstance)
            return;
        this.calendarService.getEvents(this.selectedCalendarId).subscribe((data) => {
            this.fcInstance.removeAllEvents();
            data.forEach((e) => this.fcInstance.addEvent({
                id: e.eventId,
                title: e.name,
                start: e.startDate,
                end: e.endDate,
    ...
    onUpdate() {
        if (!this.editEvent.name || !this.selectedEvent)
            return;
        this.loading = true;
        this.calendarService
            .updateEvent(this.selectedCalendarId, this.selectedEvent.id, {
    ...
    deleteAndRecreateCalendar(calendarId, calName, doctor) {
    ...
    createCalendarAndSeedDutyShifts(calName, doctor) {
        this.calendarService.createCalendar(calName, `Calendar for ${calName}`).subscribe({
            next: (newCal) => {
                const newCalId = newCal.calendarId;
                this.calendarList = this.calendarList.map((c) => (c.name === calName ? { ...c, calendarId: newCalId } : c));
    ...
    createInBatches(calendarId, events, index, onComplete) {
        const batch = events.slice(index, index + 5);
        if (!batch.length) {
            onComplete?.();
            return;
...
], HospitalCalendarComponent.prototype, "calendarEl", void 0);
HospitalCalendarComponent = __decorate([
    Component({
        selector: 'app-hospital-calendar',
        standalone: true,
        imports: [CommonModule, FormsModule, RouterModule, DialogModule, ButtonModule, InputTextModule, SelectModule, TextareaModule, ConfirmDialogModule, DatePickerModule],
...
], HospitalCalendarComponent);
export { HospitalCalendarComponent };
