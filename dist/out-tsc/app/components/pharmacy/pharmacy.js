...
let PharmacyComponent = class PharmacyComponent {
    ...
    adjustingItem = null;
    adjustForm = { adjustmentType: 'received', quantity: 0, reason: '', batchNumber: '', expiryDate: null, location: '' };
    adjustSaving = false;
    invFilter = '';
    adjustTypeOptions = [
    ...
    showPoDialog = false;
    editingPo = null;
    poForm = { supplier: '', notes: '', expectedDate: null, currency: 'QAR', items: [] };
    poSaving = false;
    ...
    loadAlerts() {
        ...
        this.pharmService
            .getAlerts()
            .pipe(catchError(() => of({ count: 0, alerts: [] })), finalize(() => {
    ...
    updatePoStatus(po, status) {
        this.pharmService.updatePurchaseOrder(po.poId, { status: status }).subscribe({
            next: () => {
    ...
    stockSeverity(item) {
        if (item.stockQty === 0 || item.isExpired)
            return 'danger';
    ...
    poStatusSeverity(s) {
        const m = { draft: 'secondary', submitted: 'info', ordered: 'warn', partially_received: 'warn', received: 'success', cancelled: 'danger' };
