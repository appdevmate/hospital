...
- The product is called **Akwadona**. The infrastructure is still named after the first hospital we built it for, **Tiryaq**.
...
| **CDK stack** | rename `TiryaqCdkStack` → `AkwadonaCdkStack` | Affects ALL nested resource logical IDs. |
| **S3 buckets** | rename `tiryaq-documents-*` → `akwadona-documents-*` | Bucket names are global; renaming = create new + copy + delete. |
| **API Gateway** | rename `tiryaq-api` → `akwadona-api` | Internal label only; URL stays (`jxz59jh15f.execute-api.…`). |
| **Cognito User Pool** | rename label `tiryaq-user-pool` → `akwadona-user-pool` | Pool ID `us-east-1_RACghntmS` is **immutable** — only the display name changes. |
| **CloudFront distribution** | tag `akwadona-frontend` | ID `E1Z1ZKYM74LVA7` stays. |
...
- For the **documents bucket** (`tiryaq-documents-…`): tenants' uploaded files. Hundreds of MB → GB. Use `aws s3 sync` (resumable, parallel).
...
- **Mitigation — phased migration:**
  - **Phase A** (this runbook 8.2–8.4): Rename the LAMBDAS while keeping stack name = `TiryaqCdkStack`. Use CDK's `functionName` override.
...
1. Edit `tiryaq-cdk-stack.ts` to point routes back to the old Lambdas.
...
| Old name | New name |
|---|---|
| `tiryaq-pharmacy` | `akwadona-pharmacy` |
| `tiryaq-bloodbank` | `akwadona-bloodbank` |
| `tiryaq-appointments` | `akwadona-appointments` |
| `tiryaq-examinations` | `akwadona-examinations` |
| `tiryaq-calendar` | `akwadona-calendar` |
| `tiryaq-document-manager` | `akwadona-document-manager` |
| `tiryaq-scribe` | `akwadona-scribe` |
| `tiryaq-admin-panel` | `akwadona-admin-panel` |
| `tiryaq-audit` | `akwadona-audit` |
| `tiryaq-operator-console` | `akwadona-operator-console` |
| `getAllPatients`, `createDoctor`, … (camelCase) | unchanged at this phase — camelCase names don't carry the "tiryaq" prefix |
| `tiryaq-documents-483176634665-us-east-1` (S3) | stays for now (Phase B) |
| `tiryaq-user-pool` (Cognito display name) | `akwadona-user-pool` (display label only) |
| `tiryaq-api` (API Gateway display) | `akwadona-api` |
