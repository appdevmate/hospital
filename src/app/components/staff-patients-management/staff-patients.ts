...
// import { Component, AfterViewInit, OnDestroy, TemplateRef, ViewChild, DestroyRef, inject, signal, computed, effect } from '@angular/core';
...
//     imports: [CommonModule, TabsModule, GenericTableComponent, TagModule, ConfirmDialogModule, ButtonModule, Tooltip],
...
//             <p-tabpanels>
//                 <p-tabpanel [value]="0">
//                     <ng-container *ngTemplateOutlet="tableArea; context: { $implicit: 'doctors' }"></ng-container>
//                 </p-tabpanel>
//                 <p-tabpanel [value]="1">
//                     <ng-container *ngTemplateOutlet="tableArea; context: { $implicit: 'patients' }"></ng-container>
//                 </p-tabpanel>
//             </p-tabpanels>
//         </p-tabs>
...
//                     <p-button icon="pi pi-trash" text severity="danger" class="ml-2"
//               (onClick)="deleteRow(row)" pTooltip="Delete"></p-button>
//                 </ng-container>
...
//     // make a mutable clone for the @Input()
//     columnsArr = computed<TableColumn[]>(() => Array.from(this.columnsSig()));

//     // tab state
//     activeIndex = 0; // 0 = patients, 1 = doctors
//     kind = signal<TabKey>('doctors');
//     onTabChange(e: number | string) {
//         const i = typeof e === 'string' ? Number(e) : e;
//         this.activeIndex = i;
...
//     private _visibleColsPatients = signal<string[]>(['name', 'gender', 'insurance', 'phone', 'qid', 'dob', 'timestamp', 'status']);
//     private _visibleColsDoctors = signal<string[]>(['name', 'gender', 'insurance', 'department', 'specialization', 'phone', 'qid', 'dob', 'timestamp', 'status']);
//     visibleColumnFields = computed<string[]>(() => (this.kind() === 'patients' ? this._visibleColsPatients() : this._visibleColsDoctors()));
...
//         if (this.kind() === 'patients') this._visibleColsPatients.set(fields);
//         else this._visibleColsDoctors.set(fields);
//     }
...
//     };
//     newBtnLabel = (k: TabKey) => (k === 'patients' ? 'New Patient' : 'New Doctor');
...
//     constructor(
//         private patientsService: PatientsService, // reuse service for both
...
//         { field: 'status', header: 'Status', editable: true, editorType: this.EditorType.Autocomplete, editorOptions: this.PATIENT_STATUS_OPTIONS, customTemplate: true, filterable: true },
...
//         { field: 'department', header: 'Department', editable: true, editorType: this.EditorType.Text, pipe: 'uppercase', filterable: true },
//         { field: 'specialization', header: 'Specialization', editable: true, editorType: this.EditorType.Text, pipe: 'uppercase', filterable: true },
//         { ...this.patientColumns[3] },
//         { ...this.patientColumns[4] },
//         { field: 'dob', header: 'Joined On', editable: true, editorType: this.EditorType.Date, type: 'date', pipe: 'date', dateFormat: 'MM/dd/yyyy', filterable: true },
//         { field: 'status', header: 'Status', editable: true, editorType: this.EditorType.Autocomplete, editorOptions: this.DOCTOR_STATUS_OPTIONS, customTemplate: true, filterable: true },
//         { ...this.patientColumns[7] }
//     ] as const;

//     readonly patientConfig: TableConfig = { ...this.baseConfig, title: 'Patients Management' };
//     readonly doctorConfig: TableConfig = { ...this.baseConfig, title: 'Doctors Management' };

//     // computed signals return the SAME references
//     columnsSig = computed<ReadonlyArray<TableColumn>>(() => (this.kind() === 'patients' ? this.patientColumns : this.doctorColumns));
//     configSig = computed<TableConfig>(() => (this.kind() === 'patients' ? this.patientConfig : this.doctorConfig));
...
//         this.helpersService.notifyInfo('Edit Mode', `Editing: ${ev.data.name || 'Unknown'}`);
//     }
//     onRowEditSave(_: RowEditEvent<Patient>) {
//         /* no-op: save happens via saveRow() */
//     }
//     onRowEditCancel(ev: RowEditEvent<Patient>) {
//         this.helpersService.notifyInfo('Edit Cancelled', `Discarded changes for ${ev.data.name || 'Unknown'}`);
//     }

//     exportCSV() {
//         const ts = new Date().toISOString().slice(0, 16).replace(/[:T]/g, '-');
//         const cols = this.columnsSig(); // computed, stable reference
//         this.tableCmp?.exportCSV({
//             selectionOnly: this._selected().length > 0,
//             filename: `${this.kind()}_${ts}`,
//             applyPipes: true,
...
//     openNew(kind: TabKey) {
//         const ref = this.dialog.open(NewPatient, { width: '50vw', modal: true, dismissableMask: true, data: { kind }, focusOnShow: false });
//         ref.onClose.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((r) => {
//             if (r) {
//                 this.helpersService.notifySuccess(`${kind === 'patients' ? 'Patient' : 'Doctor'} created`);
//                 this.refresh();
//             }
//         });
//     }
...
//             this.helpersService.notifyInfo('Warning', 'No rows selected');
//             return;
//         }
//         this.confirm.confirm({
//             key: 'global',
//             header: 'Confirm Deletion',
//             message: `Delete ${this._selected().length} selected?`,
...
//                         this._selected.set([]);
//                         if (!failed) this.helpersService.notifySuccess('Deleted');
//                         else this.helpersService.notifyError('Delete failed', 'Some items could not be deleted');
//                         this.refresh();
//                     });
//             }
//         });
...
//         this.filters = { ...this.filters, pageSize: this._pageSize(), lastKey: this.lastKey };
...
//         this.fetch(this.kind()); // same endpoint for both kinds
//     }

//     // 4) fetch(kind): branch by tab
...
//     // downloadTemplate
//     async downloadTemplate(kind: 'patients' | 'doctors'): Promise<void> {
...
//             .catch(() => {
//                 this._loading.set(false);
//                 this.helpersService.notifyError('Import failed', 'Could not read file');
//             });
//     }
...
//         if (!id) {
//             this.helpersService.notifyError('Save failed', 'Missing id');
//             return;
//         }
//         this._loading.set(true);
//         this.doctorsService.updateDoctor(id, row as unknown as CreateUpdatePatientRequest).subscribe({
//             next: () => {
//                 this._loading.set(false);
//                 api.saveRowEdit(row, rowIndex);
//                 const titlecase = new TitleCasePipe();
//                 this.helpersService.notifySuccess(`Doctor ${titlecase.transform(row?.name ?? '')} updated`);
//                 this.refresh();
//             },
//             error: () => {
//                 this._loading.set(false);
//                 api.cancelRowEdit(row, rowIndex);
//                 this.helpersService.notifyError('Update failed', 'Could not save');
//             }
//         });
//     }
...
//   const id = this.pkToId(pk);
//   if (!id) {
//     this.helpersService.notifyError('Delete failed', 'Missing id');
//     return;
//   }
...
//     key: 'global',
//     header: 'Confirm Deletion',
//     message: `Delete ${titlecase.transform(row?.name ?? '')}?`,
//     icon: 'pi pi-exclamation-triangle',
//     rejectButtonProps: { label: 'No', severity: 'secondary', variant: 'text' },
//     acceptButtonProps: { label: 'Yes', severity: 'danger' },
//     accept: () => {
...
//         next: () => { this.helpersService.notifySuccess(`${titlecase.transform(row.name)} Deleted`); this.refresh(); },
//         error: () => { this.helpersService.notifyError('Delete failed', 'Could not delete'); }
//       });
//     }
//   });
// }
...
//                 if (ok === total) this.helpersService.notifySuccess('Import completed');
//                 else if (!ok) this.helpersService.notifyError('Import failed', 'No item created');
//                 else this.helpersService.notifyError('Partial import', `${ok}/${total} created`);
//                 this.refresh();
//             });
//     }

//     private genderMap: Record<string, 'info' | 'danger' | 'secondary'> = { male: 'info', female: 'danger', na: 'secondary' };
//     private patientStatusMap: Record<string, 'success' | 'info' | 'warn' | 'danger' | 'contrast' | 'secondary'> = {
//         admitted: 'info',
//         stable: 'success',
...
//     private doctorStatusMap: Record<string, 'success' | 'info' | 'warn' | 'danger' | 'contrast' | 'secondary'> = {
//         senior: 'success',
//         junior: 'info',
...
//     // REPLACE getStatusSeverity with:
//     getStatusSeverity(v?: string) {
//         const key = (v || '').toLowerCase().trim();
//         const map = this.kind() === 'patients' ? this.patientStatusMap : this.doctorStatusMap;
//         return map[key] || 'info';
//     }
...
const GENDER_OPTIONS = [{ label: 'Male', value: 'male' }, { label: 'Female', value: 'female' }, { label: 'Prefer not to Answer', value: 'na' }] as const;
const PATIENT_STATUS_OPTIONS = [
  { label: 'ADMITTED', value: 'admitted' }, { label: 'STABLE', value: 'stable' }, { label: 'UNDER TREATMENT', value: 'under treatment' },
  { label: 'DISCHARGED', value: 'discharged' }, { label: 'CRITICAL', value: 'critical' }, { label: 'DEAD', value: 'dead' }
] as const;
const DOCTOR_STATUS_OPTIONS = [
  { label: 'SENIOR', value: 'senior' }, { label: 'JUNIOR', value: 'junior' }, { label: 'UNDER DEVELOPMENT', value: 'under development' }, { label: 'Associate', value: 'associate' }
] as const;
...
export class StaffPatientsManagement implements AfterViewInit, OnDestroy {
  ...
  private _visibleCols = {
    ...
    doctors: signal<string[]>(['name', 'gender', 'insurance', 'department', 'specialization', 'phone', 'qid', 'dob', 'timestamp', 'status'])
  };
  rows = this._rows.asReadonly();
  selected = this._selected;
  loading = this._loading.asReadonly();
  ...
  private baseConfig: TableConfig = {
    showGlobalSearch: true, showClearButton: true, showToolbar: true,
    pageSizeOptions: [5, 10, 15, 25, 50, 100], defaultPageSize: 15,
    scrollHeight: '600px', emptyMessage: 'No rows found.', loadingMessage: 'Loading...',
    showGridlines: true, rowHover: true, responsive: true, showResultsSummary: true,
    selectable: true, selectionMode: 'multiple', showSelectAll: true, editType: 'row', showColumnPicker: true
  };

  readonly patientColumns: ReadonlyArray<TableColumn> = [
    { field: 'name', header: 'Name', editable: true, editorType: EditorType.Text, pipe: 'titlecase', sortable: true, filterable: true },
    ...
    { field: 'insurance', header: 'Insurance', editable: true, editorType: EditorType.Text, pipe: 'uppercase', filterable: true },
  ...
  readonly doctorColumns: ReadonlyArray<TableColumn> = [
    ...
    { ...this.patientColumns[1] },
    { ...this.patientColumns[2] },
    { field: 'department', header: 'Department', editable: true, editorType: EditorType.Text, pipe: 'uppercase', filterable: true },
    { field: 'specialization', header: 'Specialization', editable: true, editorType: EditorType.Text, pipe: 'uppercase', filterable: true },
    { ...this.patientColumns[3] },
    { ...this.patientColumns[4] },
    { field: 'dob', header: 'Joined On', editable: true, editorType: EditorType.Date, type: 'date', pipe: 'date', dateFormat: 'MM/dd/yyyy', filterable: true },
    { field: 'status', header: 'Status', editable: true, editorType: EditorType.Autocomplete, editorOptions: DOCTOR_STATUS_OPTIONS, customTemplate: true, filterable: true },
    { ...this.patientColumns[7] }
  ] as const;

  filterControls: FilterControl[] = [
    { id: 'status', type: 'cycle', icon: 'pi pi-users', tooltip: 'Toggle status', outlined: true, values: [null, 'active', 'nonactive'],
      getLabel: (v) => (v === 'active' ? 'Non-Active' : v === 'nonactive' ? 'All' : 'Active'),
      getIcon: () => 'pi pi-users' },
    { id: 'gender', type: 'cycle', icon: 'pi pi-user', tooltip: 'Cycle gender', outlined: true, values: [null, 'male', 'female'],
  ...
  onColumnsVisibilityChange(fields: string[]) { this._visibleCols[this.kind()].set(fields); }
  ...
  onRowEditInit(ev: RowEditEvent<Patient>) { this.helpers.notifyInfo('Edit Mode', `Editing: ${ev.data.name || 'Unknown'}`); }
  onRowEditSave(_: RowEditEvent<Patient>) {}
  onRowEditCancel(ev: RowEditEvent<Patient>) { this.helpers.notifyInfo('Edit Cancelled', `Discarded changes for ${ev.data.name || 'Unknown'}`); }
  ...
  openNew(kind: TabKey) {
    const ref = this.dialog.open(NewPatient, { width: '50vw', modal: true, dismissableMask: true, data: { kind }, focusOnShow: false });
    ref.onClose.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((r) => { if (r) { this.helpers.notifySuccess(`${kind === 'patients' ? 'Patient' : 'Doctor'} created`); this.refresh(); } });
  }

  deleteSelected() {
    const sel = this._selected(); if (!sel?.length) { this.helpers.notifyInfo('Warning', 'No rows selected'); return; }
    this.confirm.confirm({
      key: 'global', header: 'Confirm Deletion', message: `Delete ${sel.length} selected?`, icon: 'pi pi-exclamation-triangle',
      rejectButtonProps: { label: 'No', severity: 'secondary', variant: 'text' }, acceptButtonProps: { label: 'Yes', severity: 'danger' },
      accept: () => {
        const ids = Array.from(new Set(sel.map(p => this.pkToId(p.PK)).filter(Boolean)));
        this._loading.set(true);
        forkJoin(ids.map(id => this.patientsService.deletePatient(id!).pipe(map(() => ({ id, ok: true as const })), catchError(() => of({ id, ok: false as const }))))) // service chooses endpoint per API GW
          .pipe(finalize(() => this._loading.set(false)))
          .subscribe(results => {
            const failed = results.filter(r => !r.ok).length;
            this._selected.set([]);
            if (!failed) this.helpers.notifySuccess('Deleted'); else this.helpers.notifyError('Delete failed', 'Some items could not be deleted');
            this.refresh();
          });
      }
    });
  ...
  load(e: any) {
    if (!e) return;
    if (e.rows && e.rows !== this._pageSize()) { this._pageSize.set(e.rows); this.reset(); }
    const sf = e.sortField ?? null, so: 1 | -1 | 0 = (e.sortOrder ?? 1);
    if (sf !== this.prevSortField || so !== this.prevSortOrder) { this.prevSortField = sf; this.prevSortOrder = so; this.reset(); }
    const first = e.first || 0;
    this.page = Math.floor(first / this._pageSize());
    this.lastKey = this.pageKeys[this.page] || null;
    this.applyFilters(e.filters || {});
  ...
  private fetch(kind: TabKey) {
    ...
    req$.subscribe({
      ...
      error: () => {
        if (seq !== this.reqSeq) return;
        this._loading.set(false); this._rows.set([]); this._totalRecords.set(0);
      }
    });
  }
  ...
  refresh() { this.fetch(this.kind()); }

  onImportFromFileInput(ev: Event) {
    const list = (ev.target as HTMLInputElement).files; if (!list?.length) return;
    this.onImport(Array.from(list)); (ev.target as HTMLInputElement).value = '';
  }
  ...
  onImport(input: File[] | { files?: File[] } | Event) {
    ...
    this._loading.set(true);
    this.readWorkbook(file).then(rows => {
      const { valid, skipped } = this.prepare(rows);
      const seen = new Set<string>();
      const unique = valid.filter(r => (seen.has(r.phone) ? false : (seen.add(r.phone), true)));
      this.confirm.confirm({
        key: 'global', header: 'Confirm Import', message: `Create ${unique.length}. Skipped ${skipped.length}. Proceed?`, icon: 'pi pi-exclamation-triangle',
        rejectButtonProps: { label: 'No', severity: 'secondary', variant: 'text' }, acceptButtonProps: { label: 'Yes', severity: 'primary' },
        accept: () => this.bulkCreate(unique), reject: () => this._loading.set(false)
      });
    }).catch(() => { this._loading.set(false); this.helpers.notifyError('Import failed', 'Could not read file'); });
  }

  saveRow(row: Patient, rowIndex: number, api: any) {
    const pk = (row as any)?.PK, id = pk ? pk.split('#')[1] || pk : undefined;
    if (!id) { this.helpers.notifyError('Save failed', 'Missing id'); return; }
    this._loading.set(true);
    this.doctorsService.updateDoctor(id, row as unknown as CreateUpdatePatientRequest).subscribe({
      next: () => { this._loading.set(false); api.saveRowEdit(row, rowIndex); this.helpers.notifySuccess(`Doctor ${new TitleCasePipe().transform(row?.name ?? '')} updated`); this.refresh(); },
      error: () => { this._loading.set(false); api.cancelRowEdit(row, rowIndex); this.helpers.notifyError('Update failed', 'Could not save'); }
    });
  }

  deleteRow(row: Patient) {
    const pk = (row as any)?.PK ?? '', id = this.pkToId(pk);
    if (!id) { this.helpers.notifyError('Delete failed', 'Missing id'); return; }
    const isDoctor = pk.startsWith('DOCTOR#') || this.kind() === 'doctors';
    this.confirm.confirm({
      key: 'global', header: 'Confirm Deletion', message: `Delete ${new TitleCasePipe().transform(row?.name ?? '')}?`, icon: 'pi pi-exclamation-triangle',
      rejectButtonProps: { label: 'No', severity: 'secondary', variant: 'text' }, acceptButtonProps: { label: 'Yes', severity: 'danger' },
      accept: () => {
        this._loading.set(true);
        const del$ = isDoctor ? this.doctorsService.deleteDoctor(id) : this.patientsService.deletePatient(id);
        del$.pipe(finalize(() => this._loading.set(false))).subscribe({
          next: () => { this.helpers.notifySuccess(`${new TitleCasePipe().transform(row.name)} Deleted`); this.refresh(); },
          error: () => { this.helpers.notifyError('Delete failed', 'Could not delete'); }
        });
      }
    });
  }

  getStatusSeverity(v?: string) { return STATUS_SEVERITY[this.kind()][(v || '').toLowerCase().trim()] || 'info'; }
  getGenderSeverity(v?: string) { return GENDER_SEVERITY[(v || '').toLowerCase().trim()] || 'secondary'; }
  ...
  private reset() { this.page = 0; this.lastKey = null; this.pageKeys = [null]; this._selected.set([]); }

  private async readWorkbook(file: File) { const { read, utils } = await import('xlsx'); const wb = read(await file.arrayBuffer(), { type: 'array' }); return utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { defval: '' }); }
  private toISODate(v: unknown): string | undefined {
    if (v == null || v === '') return undefined;
    if (v instanceof Date && !isNaN(+v)) return v.toISOString().slice(0, 10);
    if (typeof v === 'number' && isFinite(v)) { const epoch = new Date(Math.round((v - 25569) * 86400 * 1000)); return isNaN(+epoch) ? undefined : epoch.toISOString().slice(0, 10); }
    if (typeof v === 'string') {
      const s = v.trim(); if (/^\d{4}-\d{2}-\d{2}/.test(s)) return s.slice(0, 10);
      const m = s.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})$/);
      if (m) { const [, d, mo, y] = m; const yr = +y < 100 ? 2000 + +y : +y; const dt = new Date(yr, +mo - 1, +d); return isNaN(+dt) ? undefined : dt.toISOString().slice(0, 10); }
  ...
  private bulkCreate(rows: any[]) {
    from(rows).pipe(
      mergeMap(r => this.patientsService.createPatient(r).pipe(map(() => true), catchError(() => of(false))), 10),
      toArray(),
      finalize(() => this._loading.set(false))
    ).subscribe(results => {
      const ok = results.filter(Boolean).length, total = results.length;
  ...
  private pkToId(pk: string) { const m = /^PATIENTS?#(.+)$/.exec(pk); return m ? m[1] : (pk?.split('#')[1] ?? pk); }
}
