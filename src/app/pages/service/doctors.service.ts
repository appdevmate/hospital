...
export class DoctorsService {
  ...
  getDoctorsPage(opts: GetPatientsPageOpts): Observable<PagedPatientsResponse> {
  ...
  updateDoctor(patientID: string, patientData: CreateUpdatePatientRequest): Observable<{ data: Patient }> {
    return this.http.patch<{ data: Patient }>(Config.buildUrl(this.path + '/' + patientID), patientData, {
      headers: this.authHeaders()
    });
  }

  deleteDoctor(doctorID: string): Observable<{ data: Patient }> {
      return this.http.delete<{ data: Patient }>(Config.buildUrl(this.path + '/' + doctorID), {
        headers: this.authHeaders()
      });
  }
}
