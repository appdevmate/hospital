...
// Full demo using GenericTableComponent. All toolbar buttons, quick filters, and actions
// are rendered here. Row edit uses the plugin's programmatic API.
...
@Component({
    ...
    <ng-template #capEnd let-api="api" let-selected="selected">
      <p-button label="Reset Filters" class="p-button-outlined" icon="pi pi-filter-slash" (onClick)="onClearAll()"></p-button>
    </ng-template>

    <!-- actions column uses programmatic API with rowIndex -->
...
<ng-container *ngIf="!editing; else editCtrls">
  <p-button icon="pi pi-pencil" text (onClick)="api.beginRowEdit(row, rowIndex)"></p-button>
</ng-container>
...
export class TableDemo implements AfterViewInit, OnDestroy {
    ...
    loadPatients(e: any) {
        ...
        if (newSortField !== this.prevSortField || newSortOrder !== this.prevSortOrder) { this.prevSortField = newSortField; this.prevSortOrder = newSortOrder; this.reset(); }
    ...
    private fetch() {
        this.loading = true; const seq = ++this.reqSeq;
        this.patientsService.getPatientsPage(this.filters).subscribe({
            next: (r) => { if (seq !== this.reqSeq) return; this.patients = r.data || []; this.totalRecords = r.totalCount || 0; if (r.lastKey) this.pageKeys[this.page + 1] = r.lastKey; else this.pageKeys = this.pageKeys.slice(0, this.page + 1); this.loading = false; },
            error: () => { if (seq !== this.reqSeq) return; this.loading = false; this.patients = []; this.totalRecords = 0; }
        });
    }
    ...
    private applyFilters(obj: Record<string, any>) {
        ...
        for (const [k, raw] of Object.entries(obj)) {
            ...
            if (value instanceof Date) value = toYmd(value); if (typeof value === 'string') value = value.trim(); if (isBlank(value)) continue;
    ...
    private reset() { this.page = 0; this.lastKey = null; this.pageKeys = [null]; this.selectedPatients = []; }
    onSelectionChange(selectedRows: Patient[]) { this.selectedPatients = selectedRows; }

    onFilterControlChange(e: { id: string; value: any }) {
        (this.activeFilters as any)[e.id] = e.value; this.reset(); this.filters = { pageSize: this.pageSize, lastKey: null, offset: 0 };
        this.loadPatients({ first: 0, rows: this.pageSize, sortField: this.prevSortField, sortOrder: this.prevSortOrder, filters: {} });
    }

    onClearAll() { this.activeFilters = { status: null, gender: null }; this.reset(); this.filters = { pageSize: this.pageSize, lastKey: null, offset: 0 }; }

    /* Row edit */
    onRowEditInit(event: RowEditEvent<Patient>) { this.helpersFunctions.notifyInfo('Edit Mode', `Editing patient: ${event.data.name || 'Unknown'}`); }
        ...
        if (event?.data?.PK) {
            ...
            if (patientID[1]) {
                ...
                this.patientsService.updatePatient(patientID[1], patient as CreateUpdatePatientRequest).subscribe({
                    next: () => { this.loading = false; this.helpersFunctions.notifySuccess('Patient Updated Successfully!'); },
    ...
    onRowEditCancel(event: RowEditEvent<Patient>) { this.helpersFunctions.notifyInfo('Edit Cancelled', `Changes to patient ${event.data.name || 'Unknown'} were discarded`); }

    /* Actions */
    deleteSelectedPatients() {
        if (!this.selectedPatients?.length) { this.helpersFunctions.notifyInfo('Warning', 'No patients selected for deletion'); return; }
        this.confirmationService.confirm({
            key: 'global', header: 'Confirm Deletion', message: `Delete ${this.selectedPatients.length} selected patient(s)?`, icon: 'pi pi-exclamation-triangle',
            ...
            accept: () => {
                ...
                this.loading = true;
                const tasks = ids.map((id) => this.patientsService.deletePatient(id).pipe(map(() => ({ id, ok: true as const })), catchError(() => of({ id, ok: false as const }))));
                forkJoin(tasks).pipe(finalize(() => (this.loading = false))).subscribe((results) => {
                    const failed = results.filter((r) => !r.ok).length; this.selectedPatients = [];
    ...
    openNewPatient() {
        ...
        ref.onClose.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((result) => { if (result) { this.helpersFunctions.notifySuccess('Patient profile created successfully!'); this.refreshPatientTable(); } });
    }

    refreshPatientTable(): void { this.fetch(); }

    /* Import */
    onImportFromFileInput(ev: Event) { const list = (ev.target as HTMLInputElement).files; if (!list || list.length === 0) return; this.onImportPatients(Array.from(list)); (ev.target as HTMLInputElement).value = ''; }
    ...
    onImportPatients(input: File[] | { files?: File[] } | Event) {
        ...
        this.readWorkbook(file)
            .then((rows) => {
                ...
                const unique = valid.filter((r) => { if (seen.has(r.phone)) return false; seen.add(r.phone); return true; });
                this.confirmationService.confirm({
                    key: 'global', header: 'Confirm Import', message: `Create ${unique.length} patient(s). Skipped ${skipped.length}. Proceed?`, icon: 'pi pi-exclamation-triangle',
                    rejectButtonProps: { label: 'No', severity: 'secondary', variant: 'text' }, acceptButtonProps: { label: 'Yes', severity: 'primary' },
                    accept: () => this.bulkCreatePatients(unique), reject: () => { this.loading = false; }
                });
            })
            .catch(() => { this.loading = false; this.helpersFunctions.notifyError('Import failed', 'Could not read the file'); });
    }

    private async readWorkbook(file: File): Promise<any[]> { const { read, utils } = await import('xlsx'); const buf = await file.arrayBuffer(); const wb = read(buf, { type: 'array' }); const ws = wb.Sheets[wb.SheetNames[0]]; return utils.sheet_to_json(ws, { defval: '' }); }

    private preparePatients(rows: any[]) {
        const accepted = new Set(['name', 'phone', 'dob', 'gender', 'insurance', 'job', 'licenseNumber', 'qatarID', 'qid']);
        const valid: any[] = []; const skipped: any[] = [];
        for (const r of rows) {
    ...
    private bulkCreatePatients(rows: any[]) {
        from(rows)
            ...
            .subscribe((results) => {
                const ok = results.filter(Boolean).length; const total = results.length;
    ...
    private pkToId(pk: string): string { const m = /^PATIENTS?#(.+)$/.exec(pk); return m ? m[1] : (pk?.split('#')[1] ?? pk); }
