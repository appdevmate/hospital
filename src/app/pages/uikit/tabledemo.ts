...
//     const first = e.first || 0;                           // <-- offset from PrimeNG
...
//     // always include pageSize and lastKey
//     this.filters = { ...this.filters, pageSize: this.pageSize, lastKey: this.lastKey };

//     // send offset ONLY when no cursor for this page
//     if (!this.lastKey) {
//         (this.filters as any).offset = first;             // <-- add this line
//     } else {
//         delete (this.filters as any).offset;
//     }
...
export class TableDemo implements AfterViewInit, OnDestroy {
  ...
  constructor(
    private svc: PatientService,
    private confirmationService: ConfirmationService, // inject, do NOT provide here
    private messageService: MessageService,
    private helpersFunctions: Helpers
  ) {}

  ngAfterViewInit(): void {
    this.customTemplates = { status: this.statusTemplate, actions: this.actionsTemplate };
    this.pageSize = this.tableConfig.defaultPageSize ?? 15;
    this.filters.pageSize = this.pageSize;
  }
  ...
  exportCSV() { this.tableCmp?.exportCSV(); }
  ...
  clearSelection() { this.selectedPatients = []; this.tableCmp?.clearSelection(); }

  getSelectedPatientIds(): string[] { return this.selectedPatients.map(p => p.PK); }

  isPatientSelected(patient: Patient): boolean { return this.selectedPatients.some(s => s.PK === patient.PK); }
