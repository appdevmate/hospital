// tableplugin.ts
// Fully dynamic, reusable PrimeNG table plugin (Angular 2+ syntax)
// - No hardcoded toolbar buttons.
// - Toolbar and caption content injected via TemplateRef inputs.
// - Actions column customizable via TemplateRef without relying on Table DI directives.
// - Programmatic row-edit API exposed to user templates.
    ...
    >
      ...
      <ng-template pTemplate="body" let-row let-editing="editing" let-ri="rowIndex">
        <tr [pEditableRow]="row">
          ...
          <td *ngIf="showActionsCol()">
            <ng-container *ngIf="actionsTemplate; else builtInActions"
                          [ngTemplateOutlet]="actionsTemplate"
                          [ngTemplateOutletContext]="{ $implicit: row, rowIndex: ri, editing: editing, api: publicApi, selected: selectedRows }">
            </ng-container>
...
export class GenericTableComponent<T = any> implements OnChanges {
  ...
  // resolve a rendered <tr> element for rowIndex in current page
  private rowElementAt(index?: number): any | null {
  ...
  public readonly publicApi: TableApi<T> = {
    ...
    exportCSV: (opts?: ExportOptions) => this.exportCSV(opts),
    table: () => this.dt,
    getSelected: () => this.selectedRows,
    setVisibleColumns: (fields: string[]) => this.setVisibleColumns(fields),
    beginRowEdit: (row: T, rowIndex?: number) => this._beginRowEdit(row, rowIndex),
    saveRowEdit: (row: T, rowIndex?: number) => this._saveRowEdit(row, rowIndex),
    cancelRowEdit: (row: T, rowIndex?: number) => this._cancelRowEdit(row, rowIndex)
  };
  ...
  onFilterClick(ctl: FilterControl) {
    ...
    if (ctl.type === 'toggle') next = current ? null : true;
  ...
  isSelectedField(field: string): boolean { return (this.selectedColumnOptions || []).some(o => o.field === field); }
  isAllSelected(): boolean { return this.selectedColumnOptions?.length === this.columnOptions?.length; }
  ...
  /* -------- editing -------- */
  isEditingEnabled(): boolean { return this.config?.editType === 'row' || this.config?.editType === 'cell'; }
  isColumnEditable(col: TableColumn): boolean { return col.editable === true; }
  hasEditableColumns(): boolean { return this.columns.some(col => col.editable === true); }
  ...
  /* -------- selection -------- */
  onSelectionChange() { this.selectionChange.emit(this.selectedRows); this.refreshContexts(); }
  getSelectionMode(): 'single' | 'multiple' | null { return !this.config?.selectable ? null : this.config.selectionMode === 'single' ? 'single' : 'multiple'; }
  get viewColumns(): TableColumn[] { return (this.columns || []).filter(c => this.visibleFields.includes(c.field)); }
  getColSpan(): number { let base = this.viewColumns.length + (this.showActionsCol() ? 1 : 0); if (this.config?.selectable) base += 1; return base; }
  showActionsCol(): boolean { return !!this.actionsTemplate || (this.isEditingEnabled() && this.hasEditableColumns()); }
  ...
  /* -------- CSV export -------- */
  private csvEscape(v: any): string { const s = v == null ? '' : String(v); return `"${s.replace(/"/g, '""')}"`; }
  private raw(row: any, col: TableColumn): any { const v = this.val(row, col.field); if (v instanceof Date) return v.toISOString(); return v ?? ''; }
  exportCSV(opts?: ExportOptions) {
    ...
    const body = rows.map(r => cols.map(c => { const v = applyPipes ? this.display(r as any, c) : this.raw(r as any, c); return this.csvEscape(v); }).join(separator)).join('\r\n');
  ...
  custom(f: string): TemplateRef<any> | null { return this.customTemplates && this.customTemplates[f] ? this.customTemplates[f] : null; }
}
