...
export class GenericTableComponent<T = any> implements OnChanges {
    ...
    // ---- autocomplete helpers (keeps value after save) ----
    ...
    acSet(row: any, col: TableColumn, sel: any) {
        // Accept either the option object or the raw value; persist the actual value
    ...
    // Start edit: clone current row
    onRowEditInit(row: T, index: number): void {
        const key = (row as any)[this.dataKey];
        if (key !== undefined && key !== null) {
            // shallow clone is enough for flat rows
            this.clonedRows[key] = { ...(row as any) };
        }
        this.rowEditInit.emit({ data: row, index });
    }

    // Save edit: drop backup and emit
    ...
    // Cancel edit: restore backup and emit
    ...
    isRowSelected(row: T): boolean {
        ...
        return this.selectedRows.some((s) => (s as any)[key] === (row as any)[key]);
    }
    ...
    // Return a custom cell template by field name (or null if none)
