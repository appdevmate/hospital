...
if (doctorEmail) {
  ...
  try {
    ...
    return ok({ message: 'Patients retrieved successfully', data: result.patients, lastKey: result.nextKey, hasMore: result.hasMore, count: result.patients.length, totalCount, pageSize });
  } catch (err) {
...
function buildFilterExpression(filters) {
  ...
  if (filters.showDeleted) {
    // Show ONLY deactivated patients — those with a real non-empty deletedAt value
    ...
    // Default — hide deactivated patients (show only those without a deletedAt value)
  ...
  if (filters.search) {
    const ors = [];
    ['name', 'gender', 'insurance', 'status'].forEach((f, i) => { const c = buildFilterCondition(f, filters.search, names, values, `search${i}`); if (c) ors.push(c); });
...
async function scanUnsorted(filterExpression, names, values, pageSize, startKey, offset = 0) {
  const patients = []; let currentLastKey = startKey; let lastPatientKey = null; let skipped = 0; const maxIterations = 50; let iterations = 0;
  while (patients.length < pageSize && iterations++ < maxIterations) {
    ...
    for (const it of items) {
      if (it.PK?.startsWith('PATIENT#') && it.SK === 'PROFILE' && it.EntityType === 'PATIENT') {
        ...
        patients.push(normalizePatient(it)); lastPatientKey = { PK: it.PK, SK: it.SK }; if (patients.length >= pageSize) break;
      }
    }
    currentLastKey = res.LastEvaluatedKey; if (!currentLastKey) break;
  }
  let hasMore = false;
  if (patients.length === pageSize) {
    ...
    if (currentLastKey) probe.ExclusiveStartKey = currentLastKey; else if (lastPatientKey) probe.ExclusiveStartKey = lastPatientKey;
...
async function scanSorted(filterExpression, names, values, pageSize, startKey, sortField, sortOrder, cursor, offset = 0) {
  ...
  while (iterations++ < maxIterations) {
    ...
    if (currentLastKey) params.ExclusiveStartKey = currentLastKey;
    const res = await ddb.send(new ScanCommand(params));
    const items = res.Items || []; currentLastKey = res.LastEvaluatedKey;
    for (const it of items) if (it.PK?.startsWith('PATIENT#') && it.SK === 'PROFILE' && it.EntityType === 'PATIENT') buffer.push(normalizePatient(it));
    let eligible = sortArray(buffer, sortField, sortOrder); eligible = afterCursor(eligible, cursor, sortField, sortOrder);
    if (!cursor && startKey == null && offset > 0 && eligible.length > offset) eligible = eligible.slice(offset);
    if (eligible.length >= pageSize) {
      const page = eligible.slice(0, pageSize); const last = page[page.length - 1];
  ...
  let eligible = sortArray(buffer, sortField, sortOrder); eligible = afterCursor(eligible, cursor, sortField, sortOrder);
  if (!cursor && startKey == null && offset > 0 && eligible.length > offset) eligible = eligible.slice(offset);
  const page = eligible.slice(0, pageSize); const last = page[page.length - 1] || null;
...
function sortValueOf(item, field) { const v = item?.[field]; if (field === 'dob' || field === 'timestamp') { const t = v ? new Date(v).getTime() : NaN; return Number.isFinite(t) ? t : NaN; } return (v ?? '').toString().toLowerCase(); }
...
function afterCursor(sorted, cursor, field, order) {
  ...
  return sorted.filter(item => { const v = sortValueOf(item, field); if (v === curVal) return item.PK > curPk; return order === 1 ? v > curVal : v < curVal; });
}
...
async function getTotalPatients() {
  try {
    const res = await ddb.send(new GetCommand({ TableName: TABLE_NAME, Key: { PK: COUNTER_PK, SK: COUNTER_SK }, ProjectionExpression: '#t', ExpressionAttributeNames: { '#t': 'total' } }));
...
function normalizePatient(it) {
  return {
    ...
    deletedAt: it.deletedAt || null,   // ← exposed so Angular can read it
  };
}

function clampInt(val, def, min, max) { const n = parseInt(val, 10); return Number.isFinite(n) ? Math.min(Math.max(n, min), max) : def; }
function ok(body) { return { statusCode: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Headers': 'Content-Type, Authorization', 'Access-Control-Allow-Methods': 'GET,OPTIONS' }, body: JSON.stringify(body) }; }
function errResp(status, message) { return { statusCode: status, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify({ message }) }; }