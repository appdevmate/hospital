...
/* ── helpers ── */
const toPK = (id) => id.startsWith('PATIENT#') ? id : `PATIENT#${id}`;

const patientExists = async (pk) => {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log(`🗑️ Attempting to delete ${pks.length} patient(s):`, pks);

    // ── check existence ──
    const existenceResults = await Promise.all(
      pks.map(async (pk) => ({ pk, exists: await patientExists(pk) }))
    );

    const found    = existenceResults.filter(r => r.exists).map(r => r.pk);
    const notFound = existenceResults.filter(r => !r.exists).map(r => r.pk.replace('PATIENT#', ''));

    if (found.length === 0) {
      return fail(404, `No patients found for the provided IDs`);
    }
    ...
    const failedIds = failedPKs.map(pk => pk.replace('PATIENT#', ''));
    ...
    return {
      ...
      body: JSON.stringify({
        message: failedIds.length > 0
          ? `Deleted ${deletedCount} patient(s) with ${failedIds.length} failure(s)`
          : notFound.length > 0
            ? `Deleted ${deletedCount} patient(s); ${notFound.length} ID(s) not found`
