...
export class TiryaqStack extends cdk.Stack {
    constructor(scope: Construct, id: string, props?: cdk.StackProps) {
        ...
        // This means on a FRESH deploy (new account), all 6 GSIs will be
        ...
        const appClient = userPool.addClient('TiryaqAppClient', {
            ...
            oAuth: {
                ...
                callbackUrls: ['http://localhost:4200/', 'https://dlh976jc3xp7c.cloudfront.net/'],
                logoutUrls: ['http://localhost:4200/', 'https://dlh976jc3xp7c.cloudfront.net/']
            },
...
exports.handler = async (event) => {
    ...
    return { PhysicalResourceId: 'seed', Data: { count: puts.length } };
        ...
        // ─────────────────────────────────────────────────────────────────────
        // Users Lambda — 2 dummy users per group, password: Admin@1234
        // ─────────────────────────────────────────────────────────────────────
...
const USERS = [
    { username: 'admin1',      name: 'Admin One',      email: 'admin1@tiryaq.com',      group: 'Admin',       password: 'Admin@1234' },
    { username: 'admin2',      name: 'Admin Two',      email: 'admin2@tiryaq.com',      group: 'Admin',       password: 'Admin@1234' },
    { username: 'developer1',  name: 'Developer One',  email: 'dev1@tiryaq.com',        group: 'Developers',  password: 'Admin@1234' },
    { username: 'developer2',  name: 'Developer Two',  email: 'dev2@tiryaq.com',        group: 'Developers',  password: 'Admin@1234' },
    { username: 'doctor1',     name: 'Doctor One',     email: 'doctor1@tiryaq.com',     group: 'Doctors',     password: 'Admin@1234' },
    { username: 'doctor2',     name: 'Doctor Two',     email: 'doctor2@tiryaq.com',     group: 'Doctors',     password: 'Admin@1234' },
    { username: 'pharmacist1', name: 'Pharmacist One', email: 'pharmacist1@tiryaq.com', group: 'Pharmacists', password: 'Admin@1234' },
    ...
    for (const user of USERS) {
        try {
            await client.send(new AdminCreateUserCommand({
                UserPoolId: POOL, Username: user.username, MessageAction: 'SUPPRESS',
...
//         // ─────────────────────────────────────────────────────────────────────
//         // DynamoDB — Create table with all GSIs
//         // ─────────────────────────────────────────────────────────────────────
//         const table = new dynamodb.Table(this, 'HospitalTable', {
//             tableName: 'Hospital',
//             partitionKey: { name: 'PK', type: dynamodb.AttributeType.STRING },
...
//         // GSI: EntityType-index (used by most queries)
//         table.addGlobalSecondaryIndex({
//             indexName: 'EntityType-index',
//             partitionKey: { name: 'EntityType', type: dynamodb.AttributeType.STRING },
//             projectionType: dynamodb.ProjectionType.ALL
...
//         // GSI: PatientID-index (for patient-related queries)
//         table.addGlobalSecondaryIndex({
//             indexName: 'PatientID-index',
//             partitionKey: { name: 'patientId', type: dynamodb.AttributeType.STRING },
//             sortKey: { name: 'SK', type: dynamodb.AttributeType.STRING },
...
//         // ─────────────────────────────────────────────────────────────────────
//         // Cognito — Create User Pool from scratch
//         // ─────────────────────────────────────────────────────────────────────
//         const userPool = new cognito.UserPool(this, 'TiryaqUserPool', {
//             userPoolName: 'tiryaq-user-pool',
//             selfSignUpEnabled: false,
...
//                 flows: { authorizationCodeGrant: true },
//                 scopes: [cognito.OAuthScope.OPENID, cognito.OAuthScope.EMAIL, cognito.OAuthScope.PHONE, cognito.OAuthScope.PROFILE],
//                 callbackUrls: ['http://localhost:4200/', 'https://REPLACE_WITH_CLOUDFRONT_URL/'],
//                 logoutUrls: ['http://localhost:4200/', 'https://REPLACE_WITH_CLOUDFRONT_URL/']
//             },
//             accessTokenValidity: cdk.Duration.hours(1),
//             idTokenValidity: cdk.Duration.hours(1),
//             refreshTokenValidity: cdk.Duration.days(30),
...
//         // Groups
//         const groups = ['Admin', 'Developers', 'Doctors', 'Pharmacists'];
...
//         // Attach pre token trigger to user pool via CfnUserPool
//         const cfnUserPool = userPool.node.defaultChild as cognito.CfnUserPool;
//         cfnUserPool.lambdaConfig = {
//             preTokenGenerationConfig: {
//                 lambdaArn: preTokenFn.functionArn,
...
//         // Admin panel needs Cognito access
//         adminPanelFn.addToRolePolicy(
//             new iam.PolicyStatement({
//                 actions: ['cognito-idp:ListUsers', 'cognito-idp:ListUsersInGroup', 'cognito-idp:AdminDisableUser', 'cognito-idp:AdminEnableUser', 'cognito-idp:AdminSetUserPassword'],
//                 resources: [userPool.userPoolArn]
...
//         // ─────────────────────────────────────────────────────────────────────
//         // Seed Lambda — seeds DynamoDB with departments, specializations, counters
//         // ─────────────────────────────────────────────────────────────────────
//         const seedFn = new lambda.Function(this, 'TiryaqSeedFunction', {
//             functionName: 'tiryaq-seed',
//             runtime: lambda.Runtime.NODEJS_20_X,
...
//         // Custom Resource to invoke seed on deploy
...
//         // ─────────────────────────────────────────────────────────────────────
//         // Cognito Users Lambda — creates dummy users after deploy
//         // ─────────────────────────────────────────────────────────────────────
//         const usersFn = new lambda.Function(this, 'TiryaqUsersFunction', {
//             functionName: 'tiryaq-create-users',
//             runtime: lambda.Runtime.NODEJS_20_X,
...
//         const usersProvider = new cr.Provider(this, 'UsersProvider', {
//             onEventHandler: usersFn
//         });
...
//         // ─────────────────────────────────────────────────────────────────────
//         // S3 — Angular frontend bucket (auto-generated name)
//         // ─────────────────────────────────────────────────────────────────────
//         const siteBucket = new s3.Bucket(this, 'TiryaqFrontendBucket', {
//             blockPublicAccess: s3.BlockPublicAccess.BLOCK_ALL,
//             removalPolicy: cdk.RemovalPolicy.RETAIN,
...
//         // ─────────────────────────────────────────────────────────────────────
//         // Outputs — everything needed to configure Angular after deploy
//         // ─────────────────────────────────────────────────────────────────────
//         new cdk.CfnOutput(this, 'ApiUrl', { value: api.apiEndpoint, description: 'HTTP API URL — update Config.ts tiryaqUrl' });
//         new cdk.CfnOutput(this, 'CloudFrontUrl', { value: `https://${distribution.distributionDomainName}`, description: 'Frontend URL' });
//         new cdk.CfnOutput(this, 'S3BucketName', { value: siteBucket.bucketName, description: 'S3 bucket — use for ng build + s3 sync' });
//         new cdk.CfnOutput(this, 'UserPoolId', { value: userPool.userPoolId, description: 'Cognito User Pool ID — update app.config.ts authority' });
//         new cdk.CfnOutput(this, 'AppClientId', { value: appClient.userPoolClientId, description: 'Cognito App Client ID — update app.config.ts clientId' });
//         new cdk.CfnOutput(this, 'DistributionId', { value: distribution.distributionId, description: 'CloudFront Distribution ID — for cache invalidation' });
//         new cdk.CfnOutput(this, 'CognitoAuthority', { value: `https://cognito-idp.${region}.amazonaws.com/${userPool.userPoolId}`, description: 'Cognito authority URL' });
//     }
// }
